...
import { Activity, ShieldAlert, Zap, Lock, ChevronRight, Phone, ArrowRight, ActivitySquare, BrainCircuit } from 'lucide-react';
...
export default function PlatformTour() {
  ...
  return (
    <section id="platform" ref={containerRef} className="relative bg-[#020617] text-white h-[400vh] scroll-mt-20">
      <div className="sticky top-0 h-screen w-full flex items-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="relative h-[500px] md:h-[600px] w-full rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl overflow-hidden flex items-center justify-center group">
              ...
              <div className="relative w-full max-w-sm">
                ...
                <div className="bg-[#0a1128] border border-cyan-500/30 rounded-2xl p-6 shadow-[0_0_30px_rgba(34,211,238,0.15)] relative z-10">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="text-xs text-center text-cyan-400 font-mono uppercase tracking-wider">Context Unified</div>
                  </div>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: activeStep === 1 ? 1 : 0, scale: activeStep === 1 ? 1 : 0.95, pointerEvents: activeStep === 1 ? 'auto' : 'none' }}
              transition={{ duration: 0.5 }}
              className="absolute inset-0 p-8 flex flex-col items-center justify-center"
            >
                ...
                <div className="relative h-32 flex items-end justify-between gap-2 mb-6">
                  {[20, 35, 45, 60, 85].map((h, i) => (
                    <motion.div 
                      ...
                      className={`w-full rounded-t-md ${i === 4 ? 'bg-rose-500 shadow-[0_0_15px_rgba(244,63,94,0.5)]' : 'bg-white/20'}`}
                    />
                  ))}
                  {/* Threshold line */}
            ...
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: activeStep === 2 ? 1 : 0, scale: activeStep === 2 ? 1 : 0.95, pointerEvents: activeStep === 2 ? 'auto' : 'none' }}
              transition={{ duration: 0.5 }}
              className="absolute inset-0 p-8 flex flex-col items-center justify-center"
            >
              <div className="w-full max-w-sm">
                <h4 className="text-white/60 text-sm font-medium mb-4 uppercase tracking-wider text-center">Recommended Actions</h4>
                <div className="space-y-3">
                  {[
                    ...
                    <motion.button
                      ...
                      whileHover={{ scale: 1.02, backgroundColor: action.primary ? "rgba(34,211,238,0.2)" : "rgba(255,255,255,0.1)" }}
                      whileTap={{ scale: 0.98 }}
            ...
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: activeStep === 3 ? 1 : 0, scale: activeStep === 3 ? 1 : 0.95, pointerEvents: activeStep === 3 ? 'auto' : 'none' }}
              transition={{ duration: 0.5 }}
              className="absolute inset-0 p-8 flex flex-col items-center justify-center"
            >
